...
def add_soft_int_sum_constraint(
    model,
    works,
    min_max_domain,
    hard_min,
    ...
    # Penalize sums below the soft_min target.
    if soft_min > hard_min and min_cost > 0:
        delta = model.NewIntVar(-min_max_domain, min_max_domain, "")
        model.Add(delta == soft_min - sum_var)
        # TODO(user): Compare efficiency with only excess >= soft_min - sum_var.
        excess = model.NewIntVar(0, min_max_domain, prefix + ": under_sum")
        # model.AddAbsEquality(excess, delta)
        model.AddMaxEquality(excess, [delta, 0])
        cost_variables.append(excess)
        cost_coefficients.append(min_cost)
    ...
    if soft_max < hard_max and max_cost > 0:
        delta = model.NewIntVar(-min_max_domain, min_max_domain, "")
        model.Add(delta == sum_var - soft_max)
        excess = model.NewIntVar(0, min_max_domain, prefix + ": over_sum")
        # model.AddAbsEquality(excess, delta)
        model.AddMaxEquality(excess, [delta, 0])
        cost_variables.append(excess)
        cost_coefficients.append(max_cost)
