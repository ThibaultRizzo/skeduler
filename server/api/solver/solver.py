...
def solve_shift_scheduling(employees, base_shifts, period, opts=DEFAULT_OPTIONS):
    ...
    for ct in shift_constraints:
        ...
        for e in range(num_employees):
            works = [work[e, shift, d] for d in range(nb_days)]
    ...
    for ct in weekly_sum_constraints:
        ...
        for e in range(num_employees):
            for w in range(nb_weeks):
                works = [work[e, shift, d + w * nb_days] for d in range(nb_days)]
    ...
    for e in range(num_employees):
        works = [
            sum(work[e, s, d] * shift_to_hour[s] for d in range(nb_days))
            for s in range(num_shifts)
        ]
        variables, coeffs = add_soft_sum_constraint(
            model,
    ...
    for e in range(num_employees):
        for s in range(1, num_shifts):
            ...
            if coef_mastery < 2:
                num_shifts_worked = model.NewIntVar(0, nb_days, "")
                model.Add(
                    num_shifts_worked == sum(work[e, s, d] for d in range(nb_days))
                )
                if coef_mastery == 0:
    ...
    # print(len(work.items()))
    # print([solver.Value(v) for [k, v] in work.items()])
    # Print solution.
